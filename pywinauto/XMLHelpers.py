...
def _set_node_props(element, name, value):
    ...
    if isinstance(value, ctypes.Structure):
        ...
        for i, attrval in enumerate(value):
            _set_node_props(listelem, "{0}_{1:5d}".format(name, i), attrval)
...
def _escape_specials(string):
    ...
    for i in range(0, 32):
        string = string.replace(six.unichr(i), "\\{0:2d}".format(i))
...
def _un_escape_specials(string):
    ...
    for i in range(0, 32):
        string = string.replace("\\{0:2d}".format(i), six.unichr(i))
