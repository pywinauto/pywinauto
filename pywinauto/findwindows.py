...
def find_elements(class_name = None,
    ...
    * **backend**        Back-end name to use while searching (default=None means current active backend)
    ...
    if top_level_only:
        ...
        elements = element.children(process = process, class_name = class_name, title = title) # root.children == enum_windows()
    ...
    # looking for child elements
        ...
        elements = parent.descendants(process = process, class_name = class_name, title = title) # root.children == enum_windows()
