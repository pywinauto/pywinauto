...
EVENT_MAP = [
    (EventPattern(hook_event=HookEvent(current_key=HOOK_MOUSE_LEFT_BUTTON, event_type=HOOK_KEY_DOWN),
                  app_events=[
                      ApplicationEvent(name=EVENT.INVOKED)
                  ]),
     lambda: "invoked()")
]
...
class LogParser(object):
    ...
    def parse_current_log(self):
        ...
        for action in action_log:
            ...
            application_events = action.app_events
            if self.verbose:
                print("\n================================================================================\n"
                      "Hook event: {}\n    Application events: {}"
                      "".format(hook_event, application_events))

            for event_pattern, handler in EVENT_MAP:
                if event_pattern in action:
                    if hook_event.control_tree_node:
                        ...
                        script += u"app.{}.{}.{}".format(root_name, item_name, handler())
                    else:
                        # TODO: for now
